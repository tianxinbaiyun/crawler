...
import (
	...
	"github.com/tianxinbaiyun/crawler/models"
)
...
func (c *CrawlMovieController) CrawlMovie() {
	...
	for {
		...
		movieInfo.MovieName = models.GetMovieName(sMovieHtml)
		//记录电影信息
		if movieInfo.MovieName != "" {
			movieInfo.MovieDirector = models.GetMovieDirector(sMovieHtml)
			movieInfo.MovieMainCharacter = models.GetMovieMainCharacters(sMovieHtml)
			movieInfo.MovieType = models.GetMovieGenre(sMovieHtml)
			movieInfo.MovieOnTime = models.GetMovieOnTime(sMovieHtml)
			movieInfo.MovieGrade = models.GetMovieGrade(sMovieHtml)
			movieInfo.MovieSpan = models.GetMovieRunningTime(sMovieHtml)
